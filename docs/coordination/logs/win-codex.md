...
## 2026-08-28 - Phase 8 Task 11 evidence package recovery and release gate

- Branch/base/spec/plan: `feat/phase8-evaluation-evidence`; frozen base `71716a0eee8413358dfc1e125a942945fc4be18c`; spec `336be84cb4d645cd35ed198b624a2603edb5d9bd`; plan `4244ba95246b39adccfd7d97d7caa19936430b41`.
- Preserved implementation chain: `759bc1a88e92e8488d1be1b02bb8001c2977a14f`, `60e6af19771c9ee98dae11007a24f55b8f6f5270`, `4f15b67706b5489612ae51d4befee2e333f0a7a3`, `435d2778bb0bac467a8b06276a77c8c4352b524a`, `246aba3cdc9933790b7f8de56262821ffdc258ab`, `8eddcb4ccc37f514f4452064e5d23aca884c06ab`, `09326d66205743165e7f8c3a8b08aeceaca2c74a`, `e0c806246daa59eb602512199d15165b6803bb25`, `7b4ba7ad878b1765af308a50a1dedb285b59ccc6`, `8406ccb894f8851d42db1967163e47f719c59c1d`, `072ebd48720df1d8e658e7abed90e126eba2b69b`, `6174893de23d09eaefdde8c3c0c787641acdd797`, `02269691090755ea621d50c83a9c5aab448b99d3`, `4125619ff987e0af7191e6dbd44fe120c7a11fb9`, `d4bc4e7ed6a4f2c7d97b8a981a2024166e3fc950`, `f0c4c32bd34a4a1c09015f9b5fb62e112172b45c`, `5cdce26b85e2922d23fe79d23d5d8bc8909ef775` (final implementation HEAD before this Task 11 commit).
- Files changed: typed evidence contracts, composite collection/CLI/Markdown/runtime/provenance modules, small AgentRuntime provenance support, deterministic acceptance tests, generated canonical artifacts, and the approved Phase 8 spec/plan. No `web/` or frontend files changed.
- Generated report: 50 claims: 46 `VERIFIED`, 1 `NOT_ESTABLISHED`, 3 `DEFERRED`; deterministic fingerprint `d707b991f87cc865300e431594c6792766744ee69523da98c796d67f015ee543`. Two credential-free 20-repetition generations produced this same fingerprint and equal normalized deterministic evidence. The Markdown artifact is the validated generated projection of the JSON report.
- Frozen holdout: 2,500 worlds (50 seeds x 50 worlds), reproducibility key `d0dc76fb9239f4f77320f4b0a0fd5572d0b9a86a80da0448892d5336f205fe21`; scenario-aware preserved total `31272` exceeds baseline `30034`, with zero capacity violations and zero unsafe allocations. Expected-preserved delta `0.4952` (`+4.121995072251453%`).
- Dynamic R0/R1: R0 `SYN-CNT-002,004,005,010,011,012,014,015`; R1 `SYN-CNT-001,002,004,010,011,012,014,015`; `005` cancelled, `001` planned, `002/004` committed. Preserved total `601 -> 602`; expected preserved `12.02 -> 12.04`.
- Authority/tradeoff/safety: unapproved sends and incorrect request/counter fingerprints fail closed without durable mutation; carriers cannot mutate the canonical connection and the agent exposes no approval/forbidden tools. Human tradeoff selection is not agent-exposed, stale selection fails with `DynamicYardConflict` without durable mutation, and Auto Replay halts at `TRADEOFF_DECISION_REQUIRED`. Terminal AgentRun: `ESCALATED / SAFETY_REVIEW_REQUIRED`; 6 steps; tool order `pause_agent_run`, `request_expedite_feasibility`, `prepare_rta_request`, `send_authorised_rta_request`, `request_cargo_safety_review`. Safety reports `CONTRADICTION_FOUND`, blocks automation, and checker failure persists `CHECK_FAILED` with automation blocked.
- Audit/provenance: 8/8 material-action categories covered; 50 claims, 100 references, 100 provenance rows. No OpenAI API key and zero provider-client constructions in the credential-free run.
- Runtime: 20 repetitions; p50 `584.2552499999999 ms`; p95 `649.7983 ms`; `LOCAL_MACHINE_DEPENDENT`, not a production SLA or deterministic timing claim.
- Boundary: the requested 18/5/1 ledger remains `NOT_ESTABLISHED` (R1 count 8; carrier affected `SYN-CNT-017`; safety escalation `SYN-CNT-010`; only 2 complete classifications versus 24 required). Live token usage, cost, and latency remain `DEFERRED_TO_PHASE_9`; Phase 9 was not started.
- Verification: focused suite previously `79 passed`; prior full backend `477 passed, 2 skipped`. Recovery ran `uv run --python 3.12 --extra dev pytest backend/tests -q` successfully, `uv lock --check` successfully, two credential-free evidence generations, normalized-evidence comparison, and committed-artifact validation (`2 passed`, one existing Starlette deprecation warning). `git diff --check` clean.
- Final review/disposition: no OpenAI runtime dependency in the evaluation path, no frontend changes or temporary evidence artifacts retained, volatile data excluded from the deterministic fingerprint, and generated Markdown matches JSON. Critical findings: 0. Important findings: 0. Deviations: none. No merge and no Phase 9 work.

## 2026-08-28 - Phase 8 final external review correction

- External review Important finding: Phase 8 had changed `AgentRuntimeCoordinator` rejected `result_summary` semantics solely to expose the dynamic reconsideration guard reason. This violated the approved constraint to leave existing agent runtime behavior untouched.
- Disposition: restored the frozen Phase 7 behavior. Removed the Phase 8-only `_DYNAMIC_YARD_RECONSIDERATION_GUARD_ERROR` specialization, restored the inline guard `ValueError`, and restored every rejected durable-state invocation to `Tool request rejected by durable state.`.
- Evidence correction: `dynamic_evidence_precedes_carrier_mutation` now proves the same invariant from a pre-existing unhandled material assessment, attempted `prepare_rta_request`, rejected `ValueError`, expected one-step advance, the identical assessment still unhandled, and unchanged carrier-case/RTA-request counts. It does not depend on exposing an internal production error string. The focused regression also pins the frozen generic rejected summary.
- Verification: focused dynamic-yard plus directly affected AgentRuntime regressions: `19 passed` (one existing Starlette deprecation warning). Fresh full backend command `uv run --python 3.12 --extra dev pytest backend/tests -q` completed successfully. `uv lock --check` passed. Canonical artifacts regenerated twice with `OPENAI_API_KEY` absent; both fingerprints were `d707b991f87cc865300e431594c6792766744ee69523da98c796d67f015ee543` and normalized deterministic evidence was equal. Committed-artifact JSON/Markdown validation: `2 passed` (one existing warning). `git diff --check` passed.
- Final disposition: Critical findings 0; Important findings 0. No frontend change, no merge, and Phase 9 was not started.
